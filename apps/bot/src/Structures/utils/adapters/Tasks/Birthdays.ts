...
export class Birthdays extends BaseTaskLogger {
  ...
  private async getGuildBirthdayData(guild: Guild): Promise<GuildData | null> {
    const guildDoc = await this.db.birthday.findUnique({
      where: { gID: guild.id },
      include: { settings: true, subscriptions: { include: { user: true } } }
    });

    if (!guildDoc) return null;

    const birthdayLogChannel = guild.channels.cache.get(guildDoc.settings.logChannelId) as TextChannel;
    ...
    return {
      ...
      birthdayLogChannel,
      guildBirthdays: {
        id: String(guildDoc.id),
        gID: guildDoc.gID,
        birthdays: guildDoc.subscriptions.map(subscription => subscription.user)
      }
    };
  }
  ...
  private async logBirthdayList(guild: Guild, title: string, birthdays: Birthday[]) {
    ...
    if (!guildBirthdays || birthdays.length === 0) {
      ...
      await this.sendBirthdayEmbed(birthdayLogChannel, guildDoc.settings.logMessageId || null, embed);
      return;
    }
  ...
  private async sendOrUpdateEmbed(channel: TextChannel, messageId: string | null, embed: EmbedBuilder) {
    try {
      if (messageId) {
        ...
        const birthday = await this.db.birthday.findUnique({
          where: { gID: channel.guild.id },
          select: { settingsId: true }
        });
        if (!birthday) return;
        await this.db.birthdaySettings.update({
          where: { gID: channel.guild.id },
