...
export default commandModule({
  ...
  execute: async (button, { deps, params }) => {
    ...
    const welcomeSettings = await p.welcomeSettings.findUnique({
      where: {
        gID: button.guildId!
      }, 
      select: {
    ...
    if (!welcomeSettings || !welcomeSettings.enabled) {
      return await button.reply({
        flags: 64,
        content: 'I could not find welcome settings for this guild in my database or not enabled!'
      });
    }
    if (!guild) {
      return await button.reply({
    ...
    switch (params) {
      ...
      case 'bypass-verify':
        ...
        const welcome = (await button.guild?.channels.fetch(welcomeSettings.channelId)) as TextChannel;
        await p.member.delete({ where: { id: memberData.id } });
        const counts = {
        ...
        await welcomeCreate(member, member.guild.name, counts.users, welcome, {
          ...
          await member.roles.remove(guild.nonVerifiedRoleId);
        });
        break;
    }
  }
